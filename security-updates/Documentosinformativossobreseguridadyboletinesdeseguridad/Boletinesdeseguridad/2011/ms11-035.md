---
TOCTitle: 'MS11-035'
Title: 'Boletín de seguridad de Microsoft MS11-035 - Crítica'
ms:assetid: 'ms11-035'
ms:contentKeyID: 61226023
ms:mtpsurl: 'https://technet.microsoft.com/es-ES/library/ms11-035(v=Security.10)'
---

Boletín de seguridad de Microsoft MS11-035 - Crítica
====================================================
...
Esta actualización de seguridad se considera crítica para todas las ediciones compatibles de Windows Server 2003, Windows Server 2008 (excepto Itanium) y Windows Server 2008 R2 (excepto Itanium), en la que esté instalado WINS. Para obtener más información, consulte la subsección **Software afectado y no afectado** , en esta sección.

La actualización de seguridad corrige la vulnerabilidad al solucionar un error de lógica que se produce al pasar búferes como parámetros. Para obtener más información acerca de la vulnerabilidad, consulte la subsección Preguntas más frecuentes (P+F) de la entrada de vulnerabilidad específica en la sección siguiente, **Información sobre la vulnerabilidad**.
...
**Problemas conocidos.** En el [artículo 2524426 de Microsoft Knowledge Base](http://support.microsoft.com/kb/2524426) se señalan los problemas detectados que pueden experimentar los clientes al instalar esta actualización de seguridad. Se documentan asimismo las soluciones propuestas.
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|-------------------------------------|---------------------------------------------------------------------|
| [Windows Server 2003 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=0d289146-8470-45dd-a886-d1b35a2c6d4b)                                                                            | Ejecución remota de código      | Crítica                             | [MS09-039](http://technet.microsoft.com/security/bulletin/ms09-039) |
| [Windows Server 2003 x64 Edition Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=0d3e85a9-42ff-447b-ab8c-095a1c0b224c)                                                                | Ejecución remota de código      | Crítica                             | [MS09-039](http://technet.microsoft.com/security/bulletin/ms09-039) |
| [Windows Server 2003 con SP2 para sistemas con Itanium](http://www.microsoft.com/downloads/details.aspx?familyid=4c388fe8-883a-4118-a7ff-50f1e9d0a071)                                                         | Ejecución remota de código      | Crítica                             | [MS09-039](http://technet.microsoft.com/security/bulletin/ms09-039) |
| [Windows Server 2008 para sistemas de 32 bits y Windows Server 2008 para sistemas de 32 bits Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=9019a3ff-d953-4b0b-8c7a-05aa9cf65c92) \* | Ejecución remota de código      | Crítica                             | Ninguna                                                             |
| [Windows Server 2008 para sistemas x64 y Windows Server 2008 para sistemas x64 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=841130fa-4522-4079-b7eb-f8466ac3ef12) \*               | Ejecución remota de código      | Crítica                             | Ninguna                                                             |
| [Windows Server 2008 R2 para sistemas x64 y Windows Server 2008 R2 para sistemas x64 Service Pack 1](http://www.microsoft.com/downloads/details.aspx?familyid=8732b41e-2f25-4ee1-a8b7-a8b8b9e22d08) \*         | Ejecución remota de código      | Crítica                             | Ninguna                                                             |
...
**Ejecuto uno de los sistemas operativos que están enumerados en la tabla Software afectado. ¿Por qué no se me ofrece la actualización?**  
La actualización sólo se puede instalar si WINS está instalado en los sistemas operativos enumerados en la tabla Software afectado. De forma predeterminada, WINS no se instala en estos sistemas operativos, por lo que la actualización no se ofrecerá a menos que se haya instalado.
...
<th style="border:1px solid black;" >Vulnerabilidad de error de respuesta en el servicio WINS (CVE-2011-1248)</th>
...
Vulnerabilidad de error de respuesta en el servicio WINS (CVE-2011-1248)  
------------------------------------------------------------------------
  
Existe una vulnerabilidad de ejecución remota de código en el servicio de nombres Internet de Windows (WINS) debido a validaciones insuficientes de las estructuras de datos en los paquetes de red WINS especialmente diseñados enviados al servicio WINS.
  
Para ver esta vulnerabilidad como una entrada estándar en la lista de vulnerabilidades y exposiciones comunes, consulte [CVE-2011-1248](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1248) .
  
#### Factores atenuantes para la vulnerabilidad de error de respuesta en el servicio WINS (CVE-2011-1248)
  
El factor atenuante hace referencia a una opción, configuración común o práctica recomendada general, existente en un estado predeterminado, que podría reducir la gravedad de una vulnerabilidad. Los siguientes factores atenuantes podrían ser útiles en su situación:
  
-   De forma predeterminada, WINS no se instala en ningún sistema operativo afectado. Sólo los clientes que instalen manualmente este componente están afectados por este problema.
  
#### Soluciones provisionales para la vulnerabilidad de error de respuesta en el servicio WINS (CVE-2011-1248)
  
La solución provisional hace referencia a un cambio de opción o configuración que no corrige la vulnerabilidad subyacente pero ayuda a bloquear los tipos de ataque conocidos antes de aplicar la actualización. Microsoft ha probado las soluciones provisionales y señala en la explicación si una solución provisional reduce la funcionalidad
  
-   **Bloquee el puerto TCP 42 y el puerto UDP 42 en el firewall.**
...
#### Preguntas más frecuentes sobre la vulnerabilidad de error de respuesta en el servicio WINS (CVE-2011-1248)
...
Debido a un error de lógica cuando WINS trata una excepción de envío de socket, determinados valores suministrados por el usuario permanecen en un marco de pila y se reutilizan en otro contexto. Un atacante que consiguiera aprovechar esta vulnerabilidad podría dañar la memoria de un modo tal que podría provocar la ejecución remota de código. Un atacante podría obtener el control del sistema en el contexto Sistema para Windows Server 2003 o el contexto Servicio local para Windows Server 2008 y Windows Server 2008 R2.
  
**¿Qué es WINS?**  
El servicio de nombres Internet de Windows (WINS) se diseñó específicamente para admitir NetBIOS a través de TCP/IP (NetBT). WINS se necesita para cualquier entorno en el que los usuarios obtengan acceso a recursos que tengan nombres NetBIOS. Si no usa WINS en una red de ese tipo, no podrá conectarse al recurso de red remoto con su nombre NetBIOS a menos que use archivos LMHOSTS y tal vez no pueda establecer las conexiones para compartir archivos e impresión. Para obtener más información, vea el artículo de TechNet [¿Qué es WINS?](http://technet.microsoft.com/en-us/library/cc784180.aspx)
  
**¿Para qué puede utilizar un atacante esta vulnerabilidad?**  
Un atacante que consiguiera aprovechar esta vulnerabilidad podría ejecutar código arbitrario. De esta forma, un atacante podría instalar programas, ver, cambiar o eliminar datos, o crear cuentas nuevas con los permisos del servicio WINS.
  
**¿Cómo podría aprovechar un atacante la vulnerabilidad?**  
Un atacante podría aprovechar la vulnerabilidad si crea paquetes especialmente diseñados y los envía a un sistema WINS afectado.
  
**¿Qué sistemas están más expuestos a esta vulnerabilidad?**  
Los servidores WINS que se ejecutan en Windows Server 2003, Windows Server 2008 o Windows Server 2008 R2 están más expuestos a la vulnerabilidad.
  
**¿Cómo funciona esta actualización?**  
La actualización corrige el error de lógica de la infracción del contrato entre las funciones al pasar búferes como parámetros.
...
<td style="border:1px solid black;">Use la herramienta <strong>Agregar o quitar programas</strong> del Panel de control o la utilidad Spuninst.exe que se encuentra en la carpeta %Windir%\\$NTUninstallKB2524426$\Spuninst</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>Información sobre archivos</strong></td>
<td style="border:1px solid black;">Vea el <a href="http://support.microsoft.com/kb/2524426">artículo 2524426 de Microsoft Knowledge Base</a> .</td>
</tr>
<tr class="even">
...
-   Luigi Auriemma, en colaboración con [Zero Day Initiative](http://www.zerodayinitiative.com/) de [TippingPoint](http://www.tippingpoint.com/) , por informar de la vulnerabilidad de error de respuesta en el servicio WINS (CVE-2011-1248)
