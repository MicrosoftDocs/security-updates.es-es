---
TOCTitle: 'MS11-058'
Title: 'Boletín de seguridad de Microsoft MS11-058 - Crítica'
ms:assetid: 'ms11-058'
ms:contentKeyID: 61226066
ms:mtpsurl: 'https://technet.microsoft.com/es-ES/library/ms11-058(v=Security.10)'
---

Boletín de seguridad de Microsoft MS11-058 - Crítica
====================================================

Vulnerabilidades en el servidor DNS podrían permitir la ejecución remota de código (2562485)
--------------------------------------------------------------------------------------------

Publicado: martes, 9 de agosto de 2011 | Actualizado: martes, 25 de octubre de 2011
...
Esta actualización de seguridad se considera crítica para las ediciones de 32 bits y x64 de Windows Server 2008 y las ediciones x64 de Windows Server 2008 R2; e importante para todas las ediciones compatibles de Windows Server 2003. Para obtener más información, vea la subsección **Software afectado y no afectado**, en esta sección.

La actualización de seguridad corrige las vulnerabilidades al modificar la forma en que el servidor DNS trata las consultas NAPTR en memoria e inicializa los objetos en la memoria antes de su uso. Para obtener más información acerca de las vulnerabilidades, consulte la subsección Preguntas más frecuentes (P+F) de la entrada de vulnerabilidad específica en la sección siguiente, **Información sobre la vulnerabilidad**.
...
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|-------------------------------------|---------------------------------------------------------------------------|
| [Windows Server 2003 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=c2d4aa38-9241-465d-8d65-aba73e936d0f)                                                                   | Denegación de servicio          | Importante                          | [MS09-008](http://technet.microsoft.com/es-es/security/bulletin/ms09-008) |
| [Windows Server 2003 x64 Edition Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=2db4098a-f4f9-4211-b55d-5d0df1409c43)                                                       | Denegación de servicio          | Importante                          | [MS09-008](http://technet.microsoft.com/es-es/security/bulletin/ms09-008) |
| [Windows Server 2003 con SP2 para sistemas con Itanium](http://www.microsoft.com/downloads/details.aspx?familyid=1934acfa-5637-4ae9-9e9a-fc7e58930b7a)                                                | Denegación de servicio          | Importante                          | [MS09-008](http://technet.microsoft.com/es-es/security/bulletin/ms09-008) |
| [Windows Server 2008 para sistemas de 32 bits Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=ac2d5122-6f9b-46f5-9840-77aa7ff84308)\*                                        | Ejecución remota de código      | Crítica                             | Ninguna                                                                   |
| [Windows Server 2008 para sistemas x64 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=5f605f6f-3854-403d-878b-637626aef78f)\*                                               | Ejecución remota de código      | Crítica                             | Ninguna                                                                   |
| [Windows Server 2008 R2 para sistemas x64 y Windows Server 2008 R2 para sistemas x64 Service Pack 1](http://www.microsoft.com/downloads/details.aspx?familyid=6a86e2cf-4e7d-4012-88c3-6957a3842dd3)\* | Ejecución remota de código      | Crítica                             | Ninguna                                                                   |
...
**¿Por qué se ha revisado este boletín el 25de octubre de 2011?**  
Microsoft revisó este boletín para anunciar un cambio en la lógica de detección y para corregir la información de reemplazo de boletín para todas las ediciones compatibles de Windows Server 2003. No ha habido cambios en los archivos de actualización de seguridad. Los clientes que ya han actualizado correctamente sus sistemas no necesitan realizar ninguna acción.
...
<th style="border:1px solid black;" >Vulnerabilidad de consulta NAPTR en DNS (CVE-2011-1966)</th>
<th style="border:1px solid black;" >Vulnerabilidad de daños en la memoria no inicializada en DNS (CVE-2011-1970)</th>
...
Vulnerabilidad de consulta NAPTR en DNS (CVE-2011-1966)  
-------------------------------------------------------
  
Existe una vulnerabilidad de ejecución remota de código en la forma en que el servidor DNS de Windows trata una consulta NAPTR especialmente diseñada en memoria. Un atacante que consiguiera aprovechar esta vulnerabilidad podría ejecutar código arbitrario en el contexto del sistema. De esta forma, un intruso podría instalar programas; ver, cambiar o eliminar datos; o crear cuentas nuevas con todos los derechos de usuario.
  
Para ver esta vulnerabilidad como una entrada estándar en la lista de vulnerabilidades y exposiciones comunes, vea [CVE-2011-1966](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1966).
  
#### Factores atenuantes para la vulnerabilidad de consulta NAPTR en DNS (CVE-2011-1966)
  
Microsoft no ha identificado factores atenuantes para esta vulnerabilidad.
  
#### Soluciones provisionales para la vulnerabilidad de consulta NAPTR en DNS (CVE-2011-1966)
  
La solución provisional hace referencia a un cambio de opción o configuración que no corrige la vulnerabilidad subyacente pero ayuda a bloquear los tipos de ataque conocidos antes de aplicar la actualización. Microsoft ha probado las soluciones provisionales y señala en la explicación si una solución provisional reduce la funcionalidad
  
-   **Deshabilitar el servicio DNS si no se usa**
  
    **Métodointeractivo:**
  
    1.  Haga clic en **Inicio**, en **Ejecutar**, escriba **regedit** y, a continuación, haga clic en **Aceptar**.  
    2.  Expanda **HKEY\_LOCAL\_MACHINE**, **System**, **CurrentControlSet** y finalmente **Services**.  
    3.  Haga clic en **DNS**.  
    4.  Haga clic en el menú **Archivo** y, a continuación, seleccione **Exportar**.  
    5.  En el cuadro de diálogo Exportar archivo del Registro, escriba **DNS\_configuration\_backup.reg** y, a continuación, haga clic en **Guardar**.  
    6.  Haga doble clic en el valor denominado **Start** y, a continuación, cambie el campo de datos **Valor** a **4**.  
    7.  Haga clic en **Aceptar**.  
    8.  Ejecute el siguiente comando en un símbolo del sistema como administrador:  
        `scstop`` DNS`
  
    **Con una secuencia de comandos de implementación administrada:**
  
    1.  Realice una copia de seguridad de las claves del Registro desde el script de implementación administrada con el siguiente comando:  
        `regedit``/e DNS_configuration_backup.reg HKEY_LOCAL_MACHINE\``System``\``CurrentControlSet``\``Services``\DNS`  
    2.  Guarde lo siguiente en un archivo con la extensión .REG (por ejemplo, "Disable\_service\_DNSame.reg").  
        `Editor del ``Registro`` de Windows versión 5.00`  
        `[HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\DNS]`  
        `"Start"=dword:00000004`  
    3.  Ejecute el script del Registro creado en el paso 2 anterior en equipo de destino con el siguiente comando:  
    ...
    **Métodointeractivo:**
    ...
    3.  En el cuadro de diálogo Importar archivo del Registro seleccione **DNS\_configuration\_backup.reg** y, a continuación, haga clic en **Abrir**.
  
    **Con una secuencia de comandos de implementación administrada:**
  
    Restaure el Registro a su estado original mediante la ejecución del siguiente comando en el símbolo del sistema como administrador:  
    `Regedit``/s DNS_configuration_backup.reg`
  
#### Preguntas más frecuentes sobre la vulnerabilidad de consulta NAPTR en DNS (CVE-2011-1966)
  
**¿Cuál es el alcance de esta vulnerabilidad?**   
Se trata de una vulnerabilidad de ejecución remota de código.
  
**¿Cuál es la causa de esta vulnerabilidad?**   
La vulnerabilidad se debe a que un servidor DNS no autoritativo trata incorrectamente un registro NAPTR especialmente diseñado en memoria mientras se solicita de forma recursiva el registro desde el servidor DNS autoritativo.
  
**¿Cuál es el componente afectado por esta vulnerabilidad?**   
Dns.exe es el componente afectado por esta vulnerabilidad.
  
**¿Para qué puede usar un atacante esta vulnerabilidad?**   
Un atacante que consiguiera aprovechar esta vulnerabilidad podría ejecutar código arbitrario en el contexto del sistema. De esta forma, un intruso podría instalar programas; ver, cambiar o eliminar datos; o crear cuentas nuevas con todos los derechos de usuario.
  
**¿Cómo podría aprovechar un atacante la vulnerabilidad?**   
Un atacante remoto no autenticado podría aprovechar esta vulnerabilidad mediante el registro de un dominio, la creación de registro de recurso NAPTR DNS y, a continuación, el envío de una consulta NAPTR especialmente diseñada al servidor DNS de destino..
  
**¿Qué sistemas están más expuestos a esta vulnerabilidad?**   
Los servidores que ejecutan el rol DNS están más expuestos. Los servidores que no tienen habilitado este rol no están expuestos.
  
**¿Qué es un registro de recurso NAPTR DNS?**   
Un registro de recurso (RR) de puntero de autoridad de nomenclatura (NAPTR) DNS permite que DNS se use para bloquear los servicios para una amplia variedad de nombres de recursos, incluidos los URI que no estén con la sintaxis de nombre de dominio. Para obtener más información acerca de NAPTR, vea [RFC-2915](http://www.ietf.org/rfc/rfc2915.txt).
  
**¿Qué es un registro de recurso DNS?**   
Un registro de recurso (RR) DNS identifica un determinado recurso de un dominio. Para obtener más información, vea el artículo de TechNet [Cómo funciona DNS](http://technet.microsoft.com/en-us/library/dd197446(ws.10).aspx).
  
**¿Qué es un URI?**   
Un identificador uniforme de recursos (URI) es una cadena de caracteres que se usa para actuar en recursos, o identificarlos, desde Internet o a través de una red. Una dirección URL es un ejemplo típico de un URI que hace referencia a un recurso como un sitio web. Para obtener más información acerca de los URI, vea [RFC-2396](http://www.ietf.org/rfc/rfc2915.txt).
  
**¿Cómo funciona esta actualización?**   
La actualización corrige la vulnerabilidad al modificar la forma en que el servidor DNS trata las consultas NAPTR en memoria.
...
Vulnerabilidad de daños en la memoria no inicializada en DNS (CVE-2011-1970)  
----------------------------------------------------------------------------
  
Existe una vulnerabilidad de denegación de servicio en la forma incorrecta en que el servidor DNS trata un objeto en memoria que no se ha inicializado. Un atacante que aproveche esta vulnerabilidad podría provocar que el servicio del servidor DNS en el sistema de destino deje de responder.
  
Para ver esta vulnerabilidad como una entrada estándar en la lista de vulnerabilidades y exposiciones comunes, vea [CVE-2011-1970](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1970).
  
#### Factores atenuantes para la vulnerabilidad de daños en la memoria no inicializada en DNS (CVE-2011-1970)
  
Microsoft no ha identificado factores atenuantes para esta vulnerabilidad.
  
#### Soluciones provisionales para la vulnerabilidad de daños en la memoria no inicializada en DNS (CVE-2011-1970)
  
Microsoft no ha identificado soluciones provisionales para esta vulnerabilidad.
  
#### Preguntas más frecuentes sobre la vulnerabilidad de daños en la memoria no inicializada en DNS (CVE-2011-1970)
  
**¿Cuál es el alcance de esta vulnerabilidad?**   
Se trata de una vulnerabilidad de denegación de servicio.
  
**¿Cuál es la causa de esta vulnerabilidad?**   
La vulnerabilidad se debe a que el servidor DNS trata incorrectamente un objeto no inicializado en memoria mientras se bloquea un registro de recurso de un dominio no existente.
  
**¿Cuál es el componente afectado por esta vulnerabilidad?**   
Dns.exe es el componente afectado por esta vulnerabilidad.
  
**¿Para qué puede usar un atacante esta vulnerabilidad?**   
Un atacante que aproveche esta vulnerabilidad podría provocar que el servicio del servidor DNS en el sistema de destino deje de responder.
  
**¿Cómo podría aprovechar un atacante la vulnerabilidad?**   
Un atacante remoto no autenticado podría aprovechar esta vulnerabilidad mediante el envío de una consulta DNS especialmente diseñada al servidor DNS de destino para el registro de recurso de un dominio que no existe.
...
**¿Cómo funciona esta actualización?**   
La actualización corrige la vulnerabilidad al modificar la forma en que el servidor DNS inicializa los objetos en memoria antes del uso.
...
<td style="border:1px solid black;">Use la herramienta <strong>Agregar o quitar programas</strong> del Panel de control o la utilidad Spuninst.exe que se encuentra en la carpeta %Windir%\\$NTUninstallKB2562485$\Spuninst</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>Informaciónsobrearchivos</strong></td>
<td style="border:1px solid black;">Vea el <a href="http://support.microsoft.com/kb/2562485">artículo 2562485 de Microsoft Knowledge Base</a></td>
</tr>
<tr class="even">
...
-   Grischa Zengel (Zengel Medizintechnik GmbH), por informar de la vulnerabilidad de consulta NAPTR en DNS (CVE-2011-1966)
...
-   V1.1 (21 de septiembre de 2011): Se ha corregido la tabla Software afectado para quitar MS11-046 como boletín reemplazado por esta actualización en todas las ediciones afectadas de Windows Server 2003. Se trata únicamente de un cambio de detección. No había cambios en los archivos de la actualización de seguridad. Los clientes que ya han actualizado correctamente sus sistemas no necesitan realizar ninguna acción.  
-   V1.2 (25 de octubre de 2011): Se ha anunciado un cambio en la lógica de detección y se ha corregido la información de reemplazo del boletín para algunas configuraciones afectadas. No había cambios en los archivos de la actualización de seguridad. Vea las preguntas más frecuentes de la actualización para obtener más detalles.
