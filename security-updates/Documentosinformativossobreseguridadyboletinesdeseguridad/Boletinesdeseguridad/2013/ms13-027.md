...
**He aplicado la solución provisional original "Evitar que los usuarios se conecten a un dispositivo USB que ya se ha instalado en el sistema". ¿Cómo deshago los pa sos que deshabilitaron la funcionalidad de dispositivo de almacenamiento USB?**  
...
-   **Evitar que los usuarios se conecten a un dispositivo USB que ya se ha instalado en el sistema**
    ...
    **Consecuencias de la solución provisional.** Los usuarios no pueden instalar un dispositivo USB RNDIS en el equipo.
    ...
    1.  Cree copias de seguridad de las claves del Registro. Las copias de seguridad se pueden realizar mediante un script de implementación administrada con los siguientes comandos:  
        `Regedit.exe /e Usb_rndis_backup.reg`        
         ```HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\Usb_rndisRegedit.exe /e Usb_rndisx_backup.reg HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\Usb_rndisxRegedit.exe /e Usb_rndis6_backup.reg HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\Usb_rndis6```
        
    2.  Cree un archivo de texto denominado Disable\_USB\_RNDIS.reg con el siguiente contenido:
