---
TOCTitle: 'MS10-011'
Title: 'Boletín de seguridad de Microsoft MS10-011 - Importante'
ms:assetid: 'ms10-011'
ms:contentKeyID: 61225785
ms:mtpsurl: 'https://technet.microsoft.com/es-ES/library/ms10-011(v=Security.10)'
---
...
Publicado: martes, 9 de febrero de 2010 | Actualizado: miércoles, 9 de junio de 2010
...
La actualización de seguridad corrige la vulnerabilidad al modificar la forma en que se finalizan los procesos de los usuarios al cerrar la sesión. Para obtener más información acerca de la vulnerabilidad, consulte la subsección Preguntas más frecuentes (P+F) de la entrada de vulnerabilidad específica en la sección siguiente, **Información sobre la vulnerabilidad**.****
...
**Problemas conocidos.** En el [artículo 978037 de Microsoft Knowledge Base](http://support.microsoft.com/kb/978037) se señalan los problemas detectados que pueden experimentar los clientes al instalar esta actualización de seguridad. Se documentan asimismo las soluciones propuestas.
...
|--------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|-------------------------------------|-----------------------------------------------|
| [Microsoft Windows 2000 Service Pack 4](http://www.microsoft.com/downloads/details.aspx?familyid=456185b5-57d1-4fa5-a4a9-5b2006ede3ad)                 | Elevación de privilegios        | Importante                          | Ninguna                                       |
| [Windows XP Service Pack 2 y Windows XP Service Pack 3](http://www.microsoft.com/downloads/details.aspx?familyid=b9234b40-1b1c-498b-90d4-0bff347b36ee) | Elevación de privilegios        | Importante                          | Ninguna                                       |
| [Windows XP Professional x64 Edition Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=1f83bf7a-e16c-404a-89bd-f65843938299)    | Elevación de privilegios        | Importante                          | Ninguna                                       |
| [Windows Server 2003 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=2c8c4eec-255e-41d5-b1e6-f0204edcb46f)                    | Elevación de privilegios        | Importante                          | Ninguna                                       |
| [Windows Server 2003 x64 Edition Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=de0186f6-27a2-4226-b8eb-f2912710f072)        | Elevación de privilegios        | Importante                          | Ninguna                                       |
| [Windows Server 2003 con SP2 para sistemas con Itanium](http://www.microsoft.com/downloads/details.aspx?familyid=87732f7a-9339-43bc-a4e8-3da849f35b70) | Elevación de privilegios        | Importante                          | Ninguna                                       |
...
<th style="border:1px solid black;" >Vulnerabilidad de elevación de privilegios locales en CSRSS (CVE-2010-0023)</th>
...
Vulnerabilidad de elevación de privilegios locales en CSRSS (CVE-2010-0023)  
---------------------------------------------------------------------------
  
Existe una vulnerabilidad de elevación de privilegios debida a que el subsistema de tiempo de ejecución de cliente-servidor (CSRSS) de Windows no termina correctamente los procesos de usuario cuando un servidor cierra la sesión. Un atacante que consiguiera aprovechar esta vulnerabilidad podría ejecutar código arbitrario en modo kernel. De esta forma, un intruso podría instalar programas; ver, cambiar o eliminar datos; o crear cuentas nuevas con todos los derechos de usuario.
  
Para ver esta vulnerabilidad como una entrada estándar en la lista de vulnerabilidades y exposiciones comunes, consulte [CVE-2010-0023](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0023).
  
#### Factores atenuantes para la vulnerabilidad de elevación de privilegios locales en CSRSS (CVE-2010-0023)
  
El factor atenuante hace referencia a una opción, configuración común o práctica recomendada general, existente en un estado predeterminado, que podría reducir la gravedad de una vulnerabilidad. Los siguientes factores atenuantes podrían ser útiles en su situación:
  
-   Para aprovechar esta vulnerabilidad, un atacante debe de tener unas credenciales de inicio de sesión válidas y ser capaz de iniciar una sesión local. Usuarios remotos no pueden aprovechar esta vulnerabilidad.
  
#### Soluciones provisionales para la vulnerabilidad de elevación de privilegios locales en CSRSS (CVE-2010-0023)
  
Microsoft no ha identificado soluciones provisionales para esta vulnerabilidad.
  
#### Preguntas más frecuentes sobre la vulnerabilidad de elevación de privilegios locales en CSRSS (CVE-2010-0023)
...
El subsistema de tiempo de ejecución de cliente-servidor (CSRSS) de Windows no termina correctamente los procesos de usuario cuando un servidor cierra la sesión.
  
**¿Qué es el subsistema de tiempo de ejecución de cliente-servidor?**    
CSRSS es la parte correspondiente al modo de usuario del subsistema Win32. CSRSS (subsistema de tiempo de ejecución de cliente-servidor) constituye un subsistema esencial que debe estar continuamente en ejecución. CSRSS es responsable de las ventanas de consola, así como de la creación y eliminación de subprocesos.
  
**¿Para qué puede usar un atacante esta vulnerabilidad?**    
Un atacante que aprovechara esta vulnerabilidad podría ejecutar código en el nivel de privilegio de un usuario que posteriormente iniciara sesión en el sistema. Si un administrador iniciara sesión en el sistema, el código del atacante se ejecutaría con privilegios elevados. Un atacante que aprovechara esta vulnerabilidad también podría ejecutar código diseñado para supervisar las acciones de un usuario que posteriormente iniciara sesión en el sistema. Esto permitiría divulgar información confidencial u obtener acceso a los datos de los sistemas afectados a los que pudiera tener acceso el usuario que ha iniciado sesión.
  
**¿Cómo podría aprovechar un atacante la vulnerabilidad?**    
Para aprovechar esta vulnerabilidad, el atacante debería iniciar sesión en el sistema. A continuación, podría iniciar una aplicación especialmente diseñada para seguir ejecutándose después de que el atacante cerrara la sesión. Cuando un nuevo usuario inicia sesión con sus propias credenciales, el proceso del atacante podría supervisar las acciones realizadas por el usuario que acaba de iniciar sesión. Si el usuario que acaba de iniciar sesión es un administrador, la información divulgada se podría usar para tratar de sacar más provecho al sistema afectado.
...
La actualización corrige la vulnerabilidad al modificar la forma en que se finalizan los procesos de usuario al cerrar la sesión.
...
| **Implementación**                          |                                                                                                                                                                            |  
| Instalación sin intervención del usuario    | Windows2000-KB978037-x86-enu /quiet                                                                                                                                        |  
| Instalación sin reiniciar                   | Windows2000-KB978037-x86-enu /norestart                                                                                                                                    |  
| Actualizar archivo de registro              | KB978037.log                                                                                                                                                               |  
...
| **Información sobre la eliminación**        | Use la herramienta **Agregar o quitar programas** del Panel de control o la utilidad Spuninst.exe que se encuentra en la carpeta %Windir%\\$NTUninstallKB978037$\\Spuninst |  
| **Información sobre archivos**              | Vea el [artículo 978037 de Microsoft Knowledge Base](http://support.microsoft.com/kb/978037).                                                                              |  
| **Comprobación de las claves del Registro** | HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Updates\\Windows 2000\\SP5\\KB978037\\Filelist                                                                                  |
...
<td style="border:1px solid black;">Use la herramienta <strong>Agregar o quitar programas</strong> del Panel de control o la utilidad Spuninst.exe que se encuentra en la carpeta %Windir%\\$NTUninstallKB978037$\Spuninst</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>Información sobre archivos</strong></td>
<td style="border:1px solid black;">Vea el <a href="http://support.microsoft.com/kb/978037">artículo 978037 de Microsoft Knowledge Base</a>.</td>
</tr>
<tr class="even">
...
-   Matthew 'j00ru' Jurczyk y Gynvael Coldwind, de [Hispasec](http://www.hispasec.com/), por informar de la vulnerabilidad de elevación de privilegios locales en CSRSS (CVE-2010-0023)
...
-   V1.2 (9 de junio de 2010): Se ha agregado un vínculo al artículo 978037 de Microsoft Knowledge Base en Problemas conocidos en Resumen ejecutivo.
