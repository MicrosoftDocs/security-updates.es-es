---
TOCTitle: 'MS10-104'
Title: 'Boletín de seguridad de Microsoft MS10-104 - Importante'
ms:assetid: 'ms10-104'
ms:contentKeyID: 61225978
ms:mtpsurl: 'https://technet.microsoft.com/es-ES/library/ms10-104(v=Security.10)'
---

Security Bulletin

Boletín de seguridad de Microsoft MS10-104 - Importante
=======================================================
...
Esta actualización de seguridad se considera importante para todas las ediciones compatibles de Microsoft Office SharePoint Server 2007. Para obtener más información, vea la subsección **Software afectado y no afectado**, en esta sección.

La actualización corrige la vulnerabilidad al modificar la forma en que el servicio del iniciador de conversiones de documentos valida las solicitudes SOAP especialmente diseñadas. Para obtener más información acerca de la vulnerabilidad, consulte la subsección Preguntas más frecuentes (P+F) de la entrada de vulnerabilidad específica en la sección siguiente, **Información sobre la vulnerabilidad**.
...
<tbody>
<tr class="odd">
<td style="border:1px solid black;"><a href="http://www.microsoft.com/downloads/details.aspx?familyid=3c8fb9f9-7920-43ea-b618-e26dc3360c60">Microsoft Office SharePoint Server 2007 Service Pack 2 (ediciones de 32 bits)</a><br />
(KB2433089)</td>
...
<tr class="even">
<td style="border:1px solid black;"><a href="http://www.microsoft.com/downloads/details.aspx?familyid=3db09280-24bd-42e0-9ae3-02c9bf3e8ee3">Microsoft Office SharePoint Server 2007 Service Pack 2 (ediciones de 64 bits)</a><br />
(KB2433089)</td>
<td style="border:1px solid black;">Ejecución remota de código</td>
<td style="border:1px solid black;">Importante</td>
<td style="border:1px solid black;">Ninguna</td>
...
<th style="border:1px solid black;" >Vulnerabilidad de ejecución de código en solicitud con formato incorrecto (CVE-2010-3964)</th>
<th style="border:1px solid black;" >Clasificación de gravedad acumulada</th>
</tr>
</thead>
<tbody>
...
Vulnerabilidad de ejecución de código en solicitud con formato incorrecto (CVE-2010-3964)  
-----------------------------------------------------------------------------------------
  
<span></span>
Existe una vulnerabilidad de ejecución remota de código en la forma en que el servicio del iniciador de conversiones de documentos valida las solicitudes SOAP antes de su procesamiento en un servidor de SharePoint. Un atacante que consiguiera aprovechar esta vulnerabilidad podría ejecutar código arbitrario en un servidor de SharePoint afectado en el contexto de seguridad de una cuenta de invitado.
  
Para ver esta vulnerabilidad como una entrada estándar en la lista de vulnerabilidades y exposiciones comunes, consulte [CVE-2010-3964](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3964).
  
#### Factores atenuantes para la vulnerabilidad de ejecución de código en solicitud con formato incorrecto (CVE-2010-3964)
  
El factor atenuante hace referencia a una opción, configuración común o práctica recomendada general, existente en un estado predeterminado, que podría reducir la gravedad de una vulnerabilidad. Los siguientes factores atenuantes podrían ser útiles en su situación:
  
-   De forma predeterminada, el servicio del equilibrador de carga de conversiones de documentos y el servicio del iniciador de conversiones de documentos no están habilitados en Microsoft Office SharePoint Server 2007.  
-   Un atacante que consiguiera aprovechar esta vulnerabilidad podría ejecutar código como una cuenta de invitado que solo tiene derechos en un determinado directorio temporal. El contenido de este directorio temporal específico se limpia periódicamente.
  
#### Soluciones provisionales para la vulnerabilidad de ejecución de código en solicitud con formato incorrecto (CVE-2010-3964)
  
La solución provisional hace referencia a un cambio de opción o configuración que no corrige la vulnerabilidad subyacente pero ayuda a bloquear los tipos de ataque conocidos antes de aplicar la actualización. Microsoft ha probado las soluciones provisionales y señala en la explicación si una solución provisional reduce la funcionalidad
  
-   **Detener y deshabilitar el servicio del iniciador de conversiones de documentos**
  
    Ejecute el siguiente comando desde el símbolo del sistema como administrador en cada sistema que ejecute el servicio del iniciador de conversiones de documentos:  
    `sc stop dclauncher`  
    `sc config dclauncher start= disabled`
  
    **Consecuencias de la solución provisional.** Se deshabilita la conversión de documentos de Office.
...
-   **Bloquear los puertos que usa el servicio del iniciador de conversiones de documentos de Office en el firewall**
  
    Bloquee el siguiente puerto en el firewall:  
    Puerto TCP 8082
  
    Este puerto se utiliza para iniciar una conexión con el componente afectado. Al bloquear este puerto en el firewall de la empresa, para el tráfico entrante y saliente, ayudará a proteger los sistemas situados detrás de dicho firewall de intentos de aprovechar esta vulnerabilidad. Microsoft le recomienda bloquear toda la comunicación entrante no solicitada de Internet para ayudar a impedir ataques que puedan utilizar otros puertos. Para obtener más información acerca de los puertos, vea el artículo de TechNet [Asignación de puertos TCP y UDP](http://go.microsoft.com/fwlink/?linkid=21312).
    ...
    **Cómo deshacer la solución provisional.**
    ...
    Desbloquee el puerto TCP 8082 en el firewall. Para obtener más información acerca de los puertos, vea el artículo de TechNet [Asignación de puertos TCP y UDP](http://go.microsoft.com/fwlink/?linkid=21312).
  
#### Preguntas más frecuentes sobre la vulnerabilidad de ejecución de código en solicitud con formato incorrecto (CVE-2010-3964)
  
**¿Cuál es el alcance de esta vulnerabilidad?**    
Se trata de una vulnerabilidad de ejecución remota de código. Un atacante que consiguiera aprovechar esta vulnerabilidad podría ejecutar código arbitrario en el contexto de seguridad de una cuenta de invitado en el servidor de SharePoint afectado.
  
**¿Cuál es la causa de esta vulnerabilidad?**    
Esta vulnerabilidad se debe a que el servicio del iniciador de conversiones de documentos no valida correctamente las solicitudes del protocolo simple de acceso a objetos (SOAP).
  
**¿Qué es el servicio del iniciador de conversiones de documentos?**    
El servicio del iniciador de conversiones de documentos programa e inicia las conversiones de documentos en un servidor. Cuando Microsoft Office SharePoint Server 2007 pasa una solicitud de conversión de documento al servicio, el servicio del iniciador de conversiones de documentos llama al convertidor de documentos adecuado. En la mayoría de las configuraciones de granja de servidores, el servicio del iniciador de conversiones de documentos y los programas de los convertidores que inicia se deben instalar en un servidor de aplicaciones. Cada servidor de aplicaciones solo puede tener habilitado un único servicio del iniciador de conversiones de documentos.
  
El servicio del iniciador de conversiones de documentos se ejecuta con la cuenta HVU\_*NombreDeEquipo*. La cuenta HVU\_*NombreDeEquipo* es una cuenta de invitado local que se crea en el servidor de aplicaciones cuando está habilitado el servicio del iniciador de conversiones de documentos.
  
**¿Qué es el servicio del equilibrador de carga de conversiones de documentos?**    
El servicio del equilibrador de carga de conversiones de documentos equilibra las solicitudes de conversiones de documentos. El servicio del equilibrador de carga de conversiones de documentos se puede configurar en un servidor independiente o en un servidor de una granja. Cuando recibe una solicitud de convertidor de Microsoft Office SharePoint Server 2007, el servicio del equilibrador de carga de conversiones de documentos devuelve un identificador uniforme de recursos (URI) que apunta al servicio del iniciador de conversiones de documentos adecuado. SharePoint Server 2007 se conecta al iniciador especificado mediante solicitudes SOAP y solicita que se convierta el documento especificado.
  
Tenga en cuenta que al deshabilitar el servicio del equilibrador de carga de conversiones de documentos se detiene el proceso de conversión de documentos y también se deshabilita el servicio del iniciador de conversiones de documentos.
  
**¿Para qué puede utilizar un atacante esta vulnerabilidad?**    
Un atacante que consiguiera aprovechar esta vulnerabilidad podría ejecutar código arbitrario en el contexto de seguridad de un usuario invitado en el servidor de destino.
  
**¿Cómo podría aprovechar un atacante la vulnerabilidad?**    
Un atacante puede enviar una solicitud SOAP especialmente diseñada al servicio del iniciador de conversiones de documentos en un entorno de servidor de SharePoint que use el servicio del equilibrador de carga de conversiones de documentos. Cuando el servicio del iniciador de conversiones de documentos procesa la solicitud SOAP especialmente diseñada, la vulnerabilidad podría permitir la ejecución remota de código en el contexto de seguridad de una cuenta de invitado en el sistema afectado.
  
**¿Qué sistemas están más expuestos a esta vulnerabilidad?**    
Los entornos de servidor de SharePoint que usan el servicio del equilibrador de carga de conversiones de documentos están más expuestos a esta vulnerabilidad. La vulnerabilidad podría permitir la ejecución remota de código en los servidores que ejecute el servicio del iniciador de conversiones de documentos en el entorno del servidor de SharePoint.
  
**¿Cómo funciona esta actualización?**    
Esta actualización corrige la vulnerabilidad al modificar la forma en que el servicio del iniciador de conversiones de documentos valida las solicitudes SOAP especialmente diseñadas.
...
**Nota para Microsoft Office SharePoint Server 2007** La tabla de detección descrita anteriormente se basa en implementaciones de Microsoft Office SharePoint Server 2007 de un solo servidor. Las herramientas de detección no detectan la aplicabilidad de la actualización en sistemas configurados como parte de granjas de servidores de SharePoint de múltiples sistemas. Para obtener más información acerca de la implementación de actualizaciones en configuraciones independientes y de múltiples servidores, vea el artículo de TechNet [Implementar actualizaciones de software para Office SharePoint Server 2007](http://technet.microsoft.com/es-es/library/cc263467(office.12).aspx).
...
<td style="border:1px solid black;">Vea el <a href="http://support.microsoft.com/kb/2433089">artículo 2433089 de Microsoft Knowledge Base</a>.</td>
</tr>
<tr class="even">
...
-   Oleksandr Mirosh, en colaboración con [Zero Day Initiative](http://www.zerodayinitiative.com/) de [TippingPoint](http://www.tippingpoint.com/), por informar de la vulnerabilidad de ejecución de código en solicitud con formato incorrecto (CVE-2010-3964)
