---
TOCTitle: 'MS10-007'
Title: 'Boletín de seguridad de Microsoft MS10-007 - Crítica'
ms:assetid: 'ms10-007'
ms:contentKeyID: 61225781
ms:mtpsurl: 'https://technet.microsoft.com/es-ES/library/ms10-007(v=Security.10)'
---

Security Bulletin

Boletín de seguridad de Microsoft MS10-007 - Crítica
====================================================
...
La actualización de seguridad corrige la vulnerabilidad al modificar la forma en que la API ShellExecute valida los parámetros de entrada. Para obtener más información acerca de la vulnerabilidad, consulte la subsección Preguntas más frecuentes (P+F) de la entrada de vulnerabilidad específica en la sección siguiente, **Información sobre la vulnerabilidad**.
...
|--------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|-------------------------------------|-----------------------------------------------|
| [Microsoft Windows 2000 Service Pack 4](http://www.microsoft.com/downloads/details.aspx?familyid=92234237-a8eb-4ce4-bc5e-cd86feb7dbd3)                 | Ejecución remota de código      | Crítica                             | Ninguna                                       |
| [Windows XP Service Pack 2 y Windows XP Service Pack 3](http://www.microsoft.com/downloads/details.aspx?familyid=b8e7bf17-a037-4200-9ae2-2280b19766a4) | Ejecución remota de código      | Crítica                             | Ninguna                                       |
| [Windows XP Professional x64 Edition Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=b8d83f30-9cd7-4d6b-b2b9-65d0a483cb9c)    | Ejecución remota de código      | Crítica                             | Ninguna                                       |
| [Windows Server 2003 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=5cb2e203-18fb-4887-a1c9-289d86b8ba11)                    | Ejecución remota de código      | Crítica                             | Ninguna                                       |
| [Windows Server 2003 x64 Edition Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=90360537-9311-45e2-8047-9a971f90c3c3)        | Ejecución remota de código      | Crítica                             | Ninguna                                       |
| [Windows Server 2003 con SP2 para sistemas con Itanium](http://www.microsoft.com/downloads/details.aspx?familyid=d695ca9f-a1db-4c0f-94b6-7112861c28da) | Ejecución remota de código      | Crítica                             | Ninguna                                       |
...
**¿Cómo está relacionada esta actualización de seguridad con CVE-2010-0027 descrita en MS10-002?**  
La vulnerabilidad de validación de direcciones URL (CVE-2010-0027) se corrige mediante esta actualización (MS10-007) y mediante [Actualización de seguridad acumulativa para Internet Explorer (MS10-002)](http://technet.microsoft.com/security/bulletin/ms10-002), según las versiones de Microsoft Windows y de Internet Explorer que estén instaladas en el sistema del usuario.
...
<th style="border:1px solid black;" >Vulnerabilidad de validación de direcciones URL (CVE-2010-0027)</th>
...
Se trata de una vulnerabilidad de ejecución remota de código. Un atacante que aprovechara esta vulnerabilidad podría realizar acciones con los mismos derechos que el usuario que ha iniciado sesión actualmente.
  
**¿Cuál es la causa de esta vulnerabilidad?**    
Cuando una aplicación, como un explorador web, usa la función de la API ShellExecute para procesar datos especialmente diseñados, ShellExecute puede validar incorrectamente dicho flujo de datos y ejecutar un archivo binario del sistema cliente local.
...
**¿Qué es ShellExecute?**    
ShellExecute forma parte de las funciones de la interfaz de programación de aplicaciones (API) del shell de Windows (API). Realiza una operación en un archivo especificado. Esto podría significar, por ejemplo, invocar el controlador correcto para el tipo de archivo específico. En el artículo de MSDN [Función ShellExecute](http://msdn.microsoft.com/en-us/library/bb762153(vs.85).aspx) se puede encontrar más información acerca de la función ShellExecute.
  
**¿Qué es una interfaz de programación de aplicaciones (API)?**    
Una API es un conjunto de funciones que se pueden usar para trabajar con un componente, aplicación o sistema operativo. Normalmente, una API consta de uno o varios archivos DLL que proporcionan algún de funcionalidad específica.
  
**¿Para qué puede usar un atacante esta vulnerabilidad?**    
Un atacante que aprovechara esta vulnerabilidad podría ejecutar código arbitrario como el usuario que ha iniciado la sesión. Si un usuario inicia sesión con derechos de usuario administrativos, un atacante podría lograr el control completo del sistema afectado. De esta forma, un intruso podría instalar programas; ver, cambiar o eliminar datos; o crear cuentas nuevas con todos los derechos de usuario. Por tanto, los usuarios cuyas cuentas estén configuradas con pocos derechos de usuario en el sistema correrían un riesgo menor que aquellos que cuenten con derechos de usuario administrativos.
  
**¿Cómo podría aprovechar un atacante la vulnerabilidad?**    
Un atacante podría proporcionar contenido, como una página web especialmente diseñada, que podría provocar que una aplicación analizara una cadena con formato incorrecto. Cuando este contenido se envía a la llamada de la API ShellExecute, por ejemplo, como un hipervínculo, podría permitir a un atacante ejecutar un archivo binario del sistema cliente sin conocimiento del usuario que ha iniciado la sesión.
  
**¿Qué sistemas están más expuestos a esta vulnerabilidad?**    
Los sistemas cliente que exploran contenido de Internet que no es de confianza están expuestos a esta vulnerabilidad que podría permitir que se aprovechar en el contexto del usuario que ha iniciado sesión.
  
**¿Cómo funciona esta actualización?**    
Esta actualización corrige la vulnerabilidad al modificar la forma en la API ShellExecute valida los parámetros de entrada en las ediciones compatibles de Microsoft Windows 2000, Windows XP y Windows Server 2003.
...
| **Implementación**                          |                                                                                                                                                                            |  
| Instalación sin intervención del usuario    | Windows2000-KB975713-x86-ENU /quiet                                                                                                                                        |  
| Instalación sin reiniciar                   | Windows2000-KB975713-x86-ENU /norestart                                                                                                                                    |  
| Actualizar archivo de registro              | kb975713.log                                                                                                                                                               |  
...
| **Información sobre la eliminación**        | Use la herramienta **Agregar o quitar programas** del Panel de control o la utilidad Spuninst.exe que se encuentra en la carpeta %Windir%\\$NTUninstallKB975713$\\Spuninst |  
| **Información sobre archivos**              | Vea el [artículo 975713 de Microsoft Knowledge Base](http://support.microsoft.com/kb/975713).                                                                              |  
| **Comprobación de las claves del Registro** | HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Updates\\Windows 2000\\SP5\\KB975713\\Filelist                                                                                  |
...
<td style="border:1px solid black;">Use la herramienta <strong>Agregar o quitar programas</strong> del Panel de control o la utilidad Spuninst.exe que se encuentra en la carpeta %Windir%\$NTUninstallKB975713$\Spuninst</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>Información sobre archivos</strong></td>
<td style="border:1px solid black;">Vea el <a href="http://support.microsoft.com/kb/975713">artículo 975713 de Microsoft Knowledge Base</a>.</td>
</tr>
<tr class="even">
